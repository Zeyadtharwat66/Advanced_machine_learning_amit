...
if options==1:
    ...
    z=""
    if y.isalpha() and y != "()": #Checks that expression does not include any Alphabetic
        print("You Have to Write only numbers and mathematical operators !!")
        exit()
    for i in y: #Puts the number in correct format space between each number and operator
        if i.isnumeric() or i == '.':
            z+=i
        elif i in '+-*/()':
            z += f" {i} "
    ...
    if counter!=len(operations): #Checks that Count of Operators must be less than count of numbers
            print("Invalid Expression!!")
            exit()
    if (z.__contains__("(") or z.__contains__(")")) and (z.count("(")!=z.count(")")):#Check Every bracket has its closure
